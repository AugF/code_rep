...
int query(int u, int l, int r) {
    if (tr[u].l >= l && tr[u].r <= r) return tr[u].v; // 树中节点完全被包含在[l,r]中
    
    int mid = tr[u].l + tr[u].r >> 1;
    int v = 0;
    if (l <= mid) v = query(u << 1, l, r); // 和右边没有交点
...
int main() {
    int n = 0, last = 0;
    scanf("%d%d", &m, &p);
    build(1, 1, m); // 操作数量是m个，那么序列长度是m
    
    // 等于说这里实际上看的是需要操作的位置，而且不需要保证有序，直接按要求返回所需
    int x;
    char op[2]; // 字符串过滤掉空白字符
    while (m --) { // Segment Fault
        scanf("%s%d", op, &x);
        if (*op == 'Q') {
...
unordered_set, unordered_map, unordered_multiset, unordered_multimap, 哈希表
    ...
    > lower_bound(tmp.begin(), tmp.end(), num): x<=num的迭代器
    > lower_bound(tmp.begin(), tmp.end(), num): x>num的迭代器
